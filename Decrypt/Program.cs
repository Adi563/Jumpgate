    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (key[0] <= keyTo[0] && key[1] <= keyTo[1] && key[2] <= keyTo[2] && key[3] <= keyTo[3] && key[4] <= keyTo[4] && key[5] <= keyTo[5] && key[6] <= keyTo[6] && key[7] <= keyTo[7] && key[8] <= keyTo[8] && key[9] <= keyTo[9] && key[10] <= keyTo[10] && key[11] <= keyTo[11] && key[12] <= keyTo[12] && key[13] <= keyTo[13] && key[14] <= keyTo[14] && key[15] <= keyTo[15])
            {
                ...
                //var fieldAesKey = aes.GetType().GetField("m_key", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                //var mKeyAes = fieldAesKey.GetValue(aes);
                //var fieldAesHandle = fieldAesKey.FieldType.GetField("handle", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                //var pointerAes = fieldAesHandle.GetValue(mKeyAes);

                //var fieldDecryptorKey = decryptor.GetType().GetField("m_key", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                //var mKeyDecryptor = fieldDecryptorKey.GetValue(decryptor);
                //var fieldDecryptorHandle = fieldDecryptorKey.FieldType.GetField("handle", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                //fieldDecryptorHandle.SetValue(mKeyDecryptor, pointerAes);
